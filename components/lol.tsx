...
export default class BodyItems extends React.Component<state> {
  state: state = {
    url: "",
    isFetching: false,
    vData: "",
  ...
  downloadButton: React.RefObject<HTMLButtonElement>;
  constructor(props: any) {
    super(props);
    // create a ref to store the downloadButton DOM element
  ...
  handleFetch = async () => {
    ...
    const { description, length_seconds, related_videos, video_url, formats } =
      info;
    let vFormats: any[] = [];
    console.log(formats);
    formats.map((format: { itag: any; url: any; contentLength: any }) => {
      const { itag, url, contentLength } = format;
      const cInfo = itags[itag as keyof typeof itags];
    ...
    return (
      <div className="body-items">
            ...
            <div className="flex justify-center">
              <div className="input-group relative flex justify-center items-stretch w-full mb-4">
                <input
                  ...
                  name="url"
                  placeholder="https://www.youtube.com/watch?v=vZqzVoFU0QY"
                  value={url}
                  onChange={this.handleChange}
                />
                <button
                  className="btn inline-block px-6 py-2 border-2 border-blue-800 font-light text-blue-800 text-m font-menlo leading-tight uppercase rounded hover:bg-black hover:bg-opacity-5 focus:outline-none focus:ring-0 transition duration-150 ease-in-out"
        ...
        <div className="flex justify-center">
              ...
              <div className="text-gray-700 text-base mb-4">
                <div className="flex flex-wrap">
                  <div className="formats">
                    <a
                      target="_blank"
                      href={`/api/download?url=${url}&vname=${
                        info['title']
                      }&itag=${vData as any && vData?.split("_")[0]}&format=${
          ...
          btn inline-block px-2 py-2 border-2 border-blue-800 font-light text-blue-800 text-m font-menlo leading-tight uppercase rounded bg-transparent hover:bg-black hover:bg-opacity-5 focus:outline-none focus:ring-0 transition duration-150 ease-in-out"
                      name="vData"
                      value={vData}
                      onChange={this.handleChange}
                    >
                      {info['formats'].map(
                        (
                          ...
                          } ${format.size ? `(${format.size})` : ""}`}</option>
                  ...
                  <button
                    className="btn inline-block px-2 py-2 border-2 border-blue-800 font-light text-blue-800 text-m font-menlo leading-tight uppercase rounded hover:bg-black hover:bg-opacity-5 focus:outline-none focus:ring-0 transition duration-150 ease-in-out"
